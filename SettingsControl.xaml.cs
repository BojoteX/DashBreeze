...
namespace Bojote.DashBreeze
{
    ...
    public partial class SettingsControl : UserControl
    {
        ...
        void USBChangedEvent(object sender, EventArrivedEventArgs e)
        {
            ...
            // Debugging
            SimHub.Logging.Current.Info(DashBreeze.PrintObjectProperties(sender));
            SimHub.Logging.Current.Info($"BEGIN -> USBChanged for {DashBreeze.PluginName} from {sender} for {eventType} on port {portName}");
            ...
            switch (eventType)
            {
                ...
                case "__InstanceCreationEvent":
                    Dispatcher.Invoke(() =>
                    {
                        ...
                        if (SelectedDev)
                        {
                            SimHub.Logging.Current.Info($"Here we have a problem, {DashBreeze.PluginName} is not reconnecting");
